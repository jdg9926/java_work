...
public class Ex01_Set {
	public static void main(String[] args) {
		...
		// 중복을 허용하지 않는다.
		// index를 부여하지 않기 때문에 데이터가 입력된 순서대로 출력 된다는 보장이 없다.
		...
		for (int i = 0 ; i < 7; ) {
			int randomNumber = rd.nextInt(45) + 1; // 1 ~ 30 사이의 난수 생성
			if (!hs2.contains(randomNumber)) {
		...
		// 인덱스가 없기 때문에 하나만 고를수 가 없다.
		
		// HashSet을 배열형태로 변환
		// 1. Object[]
		// 형변환이 필요하다. --> 타입 안전성이 떨어진다.
